...
@section('title')
    {{ $judul ?? 'Tambah Data Startegi Peringkat' }} - Admin
@endsection
@section('content')
    <!--  BEGIN CONTENT AREA  -->
                        ...
                        <div class="statbox widget box box-shadow">
                                ...
                                <div class="row">
                                    <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                                        <h4 class="fw-bolder">
                                            ...
                                            {{ $judul ?? 'Tambah Data Startegi Peringkat' }}
                                        </h4>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="widget-content widget-content-area">
                                ...
                                @if (Request::segment(4) == 'ubah')
                                    <form action="{{ route('dashboard.peringkat.update', $data->id) }}" method="post"
                                        enctype="multipart/form-data" class="row g-3">
                                        @method('PUT')
                                    @else
                                        <form action="{{ route('dashboard.peringkat.store') }}" method="post"
                                            enctype="multipart/form-data" class="row g-3">
                                @endif
                                @csrf
                                <div class="col-md-8">
                                 ...
                                 <div class="col-md-8">
                                    <div class="form-group mb-3">
                                        <label for="peringkat"> Peringkat
                                        </label>
                                        <select class="form-control" aria-label="Default select example" name="peringkat"
                                            @if (Request::segment(3) == 'detail') {{ 'disabled' }} @endif>
                                                @for ($x = 1; $x<= $distrik; $x++ )
                                                    <option @if (isset($data) && $data->peringkat == $x)
                                                        selected @endif value="{{$x}}"> {{$x}} </option>
                                                @endfor

                                        </select>
                                        @if ($errors->has('peringkat'))
                                            <label class="text-danger">
                                                {{ $errors->first('peringkat') }}
                                            </label>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="col-md-12">
                                    @if (Request::segment(3) == 'detail')
                                        <a href="{{ route('dashboard.peringkat.ubah', $data->id) }}" class="btn btn-primary">
                                            Ubah Data</a>
